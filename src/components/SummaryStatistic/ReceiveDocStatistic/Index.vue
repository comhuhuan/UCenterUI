<template>
	<el-tabs type="border-card" tab-position="bottom" :activeName="activeName" >
		<el-tab-pane label="当前页" :closable="false" name="当前页">
				...
				<el-form-item label="接收数据类型：" prop="commandTypeQuery">
					<el-select size="mini" v-model="fromFilters.commandTypeQuery" placeholder="请选择接收数据类型">
					    <el-option
				...
				<el-form-item label="统计图类型：" prop="chartTypeQuery">
					<el-select size="mini" v-model="fromFilters.chartTypeQuery" placeholder="请选择统计图类型">
					    <el-option
			...
			<el-table :data="tableList" border :loading="isLoading">
			    <el-table-column label="监测日志"></el-table-column>
			    <el-table-column label="过滤日志"></el-table-column>
			    <el-table-column label="活跃资源"></el-table-column>
			    <el-table-column label="活动状态"></el-table-column>
			    <el-table-column label="链路状态"></el-table-column>
			    <el-table-column label="网卡状态"></el-table-column>
...
<script>
	import { getReceiveDocStatisticList,getCommandType } from 'api/wbj/receiveDocStatistic'
	
	export default {
		name: 'ReceiveDocStatistic',
		data() {
			return {
        ...
        selectEvt(){
			...
			getList() {
				this.isLoading = true
				getReceiveDocStatisticList(this.fromFilters).then( res => {
					if(res.status === 'success') {
