<template lang="html">
    <section class="importTpl">
        ...
        <el-form label-width="120px" class="commonForm" :model="formFilter" :rules="formRules" ref="formFilter" :loading="formLoading">
            <fieldset class="expand active">
                <legend>导入</legend>
                <el-form-item label="模板：" class="tpl">
                    <a href="static/template/机房信息.xls">下载模板</a>
                </el-form-item>
                <el-form-item label="监测名称：" prop="name" :rules="{required: true, trigger: 'blur', message: '请输入监测名称'}">
                    <el-input size="mini" v-model="formFilter.name" placeholder="请输入监测名称"></el-input>
                </el-form-item>
                <el-form-item label="监测原因：" prop="reason" :rules="{required: true, trigger: 'blur', message: '请输入监测原因'}">
                    <el-input size="mini" v-model="formFilter.reason" placeholder="请输入监测原因"></el-input>
                </el-form-item>
                ...
                <el-form-item label="监测种类：" prop="rules" :rules="[
                ...
                >
                    <el-tooltip effect="dark" content="最多选择6种组合类型，其中源IP和目的IP不能同时勾选，源端口和目的端口不能同时勾选，且源端口或目的端口必须与传输层协议同时选择！" placement="top-start">
                        <el-select size="mini" multiple v-model="formFilter.rules" placeholder="请选择监测种类">
                ...
                >
                    <el-upload class="upload"
                        ...
                        :data="formFilter"
                        action="/act-idc-web/baseData/infoMonitoring/import.do"
...
import { importDo } from 'api/gjg/infoMonitoring'
    ...
    methods: {
        ...
        importHandle() {
            ...
            this.validateHandle( 'formFilter', _ => {
                ...
                importDo(this.formFilter).then(res => {
                    this.$message({
                        showClose: true,
                        message: res.status === 'success' ? '导入成功' : res.message,
