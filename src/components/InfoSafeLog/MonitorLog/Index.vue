<template>
	<el-tabs type="border-card" tab-position="bottom" :activeName="activeName" >
		<el-tab-pane label="当前页" :closable="false" name="当前页">
				...
				<el-form-item label="报警信息：" prop="keyWordQuery">
				...
				<el-form-item label="域名：" prop="hostQuery">
					...
					<el-form-item label="时间段：" prop="selectTimeQuery">
					<el-input size="mini" v-model="fromFilters.selectTimeQuery" placeholder="请选择时间段"></el-input>
				</el-form-item>
				<el-form-item label="封堵状态：" prop="isStopQuery">
					<el-select size="mini" v-model="fromFilters.isStopQuery" placeholder="请选择封堵状态">
					    <el-option
			...
			<el-table :data="tableList" border :loading="isLoading">
			    ...
			    <el-table-column label="报警信息" prop="blockReason"></el-table-column>
			    ...
			    <el-table-column label="报警时间" prop="curTime"></el-table-column>
...
<script>
	import { getMonitorLogList,getServiceCode } from 'api/wbj/monitorLog'
	...
	export default {
        ...
        selectEvt(){
			...
			getList() {
				this.isLoading = true
				getMonitorLogList(this.fromFilters).then( res => {
					if(res.status === 'success') {
