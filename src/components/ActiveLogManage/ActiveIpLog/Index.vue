	...
	<el-tabs type="border-card" tab-position="bottom" :activeName="activeName" >
		<el-tab-pane label="当前页" :closable="false" name="当前页">
				...
				<el-form-item label="传输层协议类型：" prop="protocolQuery">
					<el-select size="mini" v-model="fromFilters.protocolQuery" placeholder="请选择">
					    <el-option
...
<script>
	import { getActiveIpLogList,getServiceCode } from 'api/wbj/activeIpLog'
	...
	export default {
        ...
        selectEvt(){
			...
			getList() {
				this.isLoading = true
				getActiveIpLogList(this.fromFilters).then( res => {
					if(res.status === 'success') {
